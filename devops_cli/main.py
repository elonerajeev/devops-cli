...
def status():
    ...
    console.print(Panel(header_text, box=box.DOUBLE, border_style="cyan", padding=(0, 2)))
    ...
    # Users
    if summary["users"]:
        table.add_row("Users", "[green]✓ Configured[/green]", str(counts["users"]), "devops admin user-list")
    else:
        table.add_row("Users", "[yellow]! Not Set[/yellow]", "0", "devops admin user-add")

    # Apps
    if summary["apps"]:
        table.add_row("Applications", "[green]✓ Configured[/green]", str(counts["apps"]), "devops app list")
    else:
        table.add_row("Applications", "[yellow]! Not Set[/yellow]", "0", "devops admin app-add")

    # Servers
    if summary["servers"]:
        table.add_row("SSH Servers", "[green]✓ Configured[/green]", str(counts["servers"]), "devops ssh list")
    else:
        table.add_row("SSH Servers", "[yellow]! Not Set[/yellow]", "0", "devops admin server-add")

    # AWS Roles
    if summary["aws_roles"]:
        table.add_row("AWS Roles", "[green]✓ Configured[/green]", str(counts["aws_roles"]), "devops admin aws-list")
    else:
        table.add_row("AWS Roles", "[dim]- Optional[/dim]", "0", "devops admin aws-add-role")

    # Monitoring
    if summary["monitoring"]:
        table.add_row("Monitoring", "[green]✓ Configured[/green]", str(counts["monitoring_resources"]), "devops monitor")
    else:
        table.add_row("Monitoring", "[dim]- Optional[/dim]", "0", "devops monitor add-website")
    ...
    try:
        ...
        if session:
            console.print(f"[green]✓[/green] Logged in as: [cyan]{session.get('email')}[/cyan] ({session.get('role')})")
        else:
            console.print("[yellow]![/yellow] Not logged in. Run: [cyan]devops auth login[/cyan]")
    except:
...
def doctor():
    ...
    console.print()
    console.print(Panel("[bold]DevOps CLI Doctor[/bold]", box=box.DOUBLE, border_style="cyan"))
    console.print()
    ...
    packages = [("typer", "typer"), ("rich", "rich"), ("yaml", "pyyaml"), ("httpx", "httpx"), ("boto3", "boto3")]
    ...
    if config_dir.exists():
        ...
        console.print(f"[yellow]![/yellow] Config directory not found")
    ...
    try:
        ...
        if result.returncode == 0:
            console.print(f"[green]✓[/green] AWS CLI installed")
        else:
